...
cp -rf rs-proxy /usr/local/bin/rs-proxy && chmod +x /usr/local/bin/rs-proxy && sed -i 's/SED_SOCKS5_SERVER/'${SOCKS5_SERVER}'/g' /usr/local/bin/rs-proxy && sed -i 's/SED_PROXY_PORT/'${PROXY_PORT}'/g' /usr/local/bin/rs-proxy
cp -rf rs-proxyall /usr/local/bin/rs-proxyall && chmod +x /usr/local/bin/rs-proxyall && sed -i 's/SED_SOCKS5_SERVER/'${SOCKS5_SERVER}'/g' /usr/local/bin/rs-proxyall && sed -i 's/SED_PROXY_PORT/'${PROXY_PORT}'/g' /usr/local/bin/rs-proxyall
cp -rf rs-restart /usr/local/bin/rs-restart && chmod +x /usr/local/bin/rs-restart
